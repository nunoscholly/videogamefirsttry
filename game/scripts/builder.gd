...
func _ready():
	...
	for structure in structures:
		...
		var mesh_data = get_mesh_data(structure.model)

		mesh_library.create_item(id)
		mesh_library.set_item_mesh(id, mesh_data.mesh)
		mesh_library.set_item_mesh_transform(id, mesh_data.transform)

	gridmap.mesh_library = mesh_library

	SimState.state_changed.connect(update_cash)
	update_structure()
	update_cash()
...
# Retrieve the mesh and its local transform from a PackedScene's first
# MeshInstance3D — lets procedural items offset the mesh vertically so it sits
# on the ground inside a GridMap cell.

func get_mesh_data(packed_scene) -> Dictionary:
	var result := {"mesh": null, "transform": Transform3D()}
	...
	for i in range(scene_state.get_node_count()):
		...
		for j in scene_state.get_node_property_count(i):
			...
			if prop_name == "mesh":
				result.mesh = prop_value.duplicate()
			elif prop_name == "transform":
				result.transform = prop_value
		return result
	return result
...
func action_build(gridmap_position):
	if Input.is_action_just_pressed("build"):
		...
		if previous_tile == index:
			return  # already this structure here, nothing to do (no cost, no event)

		var cost: int = structures[index].price
		if not SimState.try_spend(cost):
...
# Used by the shop UI to jump directly to a specific structure.
func select_structure(idx: int) -> void:
	if idx < 0 or idx >= structures.size():
		return
	if idx == index:
		return
	index = idx
...
func update_cash():
	cash_display.text = "$" + str(SimState.treasury)
